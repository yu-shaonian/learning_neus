...
class Dataset:
    def __init__(self, is_train, **kwargs):
        ...
        self.render_cameras_name = kwargs['render_cameras_name']
        self.object_cameras_name = kwargs['object_cameras_name']
        ...
        self.images_np = np.stack([cv2.imread(im_name) for im_name in self.images_lis]) / 256.0
        self.masks_lis = sorted(glob(os.path.join(self.data_dir, 'mask/*.png')))
        self.masks_np = np.stack([cv2.imread(im_name) for im_name in self.masks_lis]) / 256.0
    ...
    def __getitem__(self, img_idx):
        ...
        # import ipdb
        # ipdb.set_trace()
        p = torch.matmul(self.intrinsics_all_inv[img_idx, None, :3, :3].to(p.device), p[:, :, None]).squeeze() # batch_size, 3
        rays_v = p / torch.linalg.norm(p, ord=2, dim=-1, keepdim=True)    # batch_size, 3
        rays_v = torch.matmul(self.pose_all[img_idx, None, :3, :3].to(p.device), rays_v[:, :, None]).squeeze()  # batch_size, 3
        rays_o = self.pose_all[img_idx, None, :3, 3].expand(rays_v.shape).to(p.device) # batch_size, 3
        near, far = self.near_far_from_sphere(rays_o, rays_v)
        batch = {}
        batch['rays_o'] = rays_o
        batch['rays_d'] = rays_v
